...
export default function AreaChart({ data, activeDay, setActiveDay }: AreaChartProps) {
  ...
  const points = data.map((d, i) => {
    ...
    const y = CHART_HEIGHT - ((d.amount - minVal) / range) * (CHART_HEIGHT - 30) - 15;
    return { x, y, value: d.amount, day: d.day };
  });

  // 1. Draw the Area Fill (interpolated vertical bars to simulate a gradient polygon)
  const fillBars = [];
  const steps = 60;
    ...
    fillBars.push(
      <View
        ...
        style={{
          ...
          backgroundColor: 'rgba(56,189,248,0.2)', // Translucent blue fill
        }}
      />
    );
  }
          ...
          >
            {activeDay === i && (
              <View style={styles.tooltip}>
                <Text style={styles.tooltipText}>₹{p.value.toFixed(0)}</Text>
